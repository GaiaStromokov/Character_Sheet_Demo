...
def gRace(): return shared.db.Core.R.replace(" ", "")
def gSubrace(): return shared.db.Core.SR.replace(" ", "")
def gRace_data(): return shared.db.Race
def gRace_abil(): return shared.db.Race["Abil"]


def gClass(): return shared.db.Core.C.replace(" ", "")
def gSubclass(): return shared.db.Core.SC.replace(" ", "")
...
def gBackground(): return shared.db.Core.BG.replace(" ", "")
...
def set_Milestone_Feat_Choice(sender, data, user_data):
    feat = user_data[0]
    index = user_data[1]
    if data == "Clear": output = ""
    else: output = data
    
    if feat in list(gMilestone()["Data"].keys()):
        gMilestone()["Data"][feat]["Select"][index] = output
...
def set_Player_HP_Mod(sender, data, user_data):
    gHP()["Player"] = int(data)
