...
def get_valid_class():
    return bool(shared.db.Valid.Class)
...
def can_cast():
    return gClass() in spellcast_L or gSubclass() in spellcast_L

def get_cantrip_known():
    return len(gSpell().Book[0])
...
def get_condition(item):
    return gCondition()[item]

def get_conditionc(item):
    return c_item_true if get_condition(item) else c_item_false
...
def get_profc(cat,item):
    return c_item_true if item in gProf()[cat] else c_item_false
...
def set_Health_Mod(sender, data, user_data):
    place, sign, delta = user_data
    hp = shared.db.Health["HP"]
...
def set_Milestone_Lselect(sender, data, user_data):
    pass
    # if data == "Feat":
    #     gMilestone()[user_data[0]]={"Type": "Feat", "Name": ""}
    # if data == "Asi":
    #     db.Milestone[user_data[0]]={"Type": "Asi", "Select": ["",""]}
